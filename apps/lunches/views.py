...
class LunchAPIView(APIView):
  permission_classes = [IsAuthenticated]
  parser_classes = [JSONParser, FormParser, MultiPartParser]

  def get(self, request, pk=None, format=None):
    if pk:
      obj = get_object_or_404(Lunch, pk=pk, deleted_at__isnull=True)
      serializer = LunchSerializer(obj)
    ...
    objs = Lunch.objects.filter(
      deleted_at__isnull=True,
      date__range=(start_of_week, end_of_week)
    ).order_by('-date')
    
    serializer = LunchSerializer(objs, many=True)
  ...
  def put(self, request, pk, format=None):
    obj = get_object_or_404(Lunch, pk=pk, deleted_at__isnull=True)
    serializer = LunchSerializer(obj, data=request.data)
    if serializer.is_valid():
  ...
  def patch(self, request, pk, format=None):
    obj = get_object_or_404(Lunch, pk=pk, deleted_at__isnull=True)
    serializer = LunchSerializer(obj, data=request.data, partial=True)
    if serializer.is_valid():
      serializer.save(updated_at=timezone.now())
      return Response(serializer.data, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
  ...
  def _delete_multiple(self, request):
    try:
      ...
      existing_objs = Lunch.objects.filter(
        id__in=ids,
        deleted_at__isnull=True
      )
